//
// @File: GXInuclNuclei.h
// Purpose: Vectorized version of class representing nucleus-like targets (or projectiles)
//
// 20180611  Guilherme Lima -- Created, based on M.Kelsey's G4InuclNuclei

#ifndef GXBERT_GXInuclNuclei_H
#define GXBERT_GXInuclNuclei_H
...
template <typename T>
class GXInuclNuclei : public GXInuclParticle<T> {
...
public:
  ...
  VECCORE_ATT_HOST_DEVICE
  VECCORE_FORCE_INLINE
  GXInuclNuclei(Index_v<T> ityp, Model model = DefaultModel)
    : BaseClass(model)
    , iType(ityp)
  { }
  ...
  VECCORE_ATT_HOST_DEVICE
  VECCORE_FORCE_INLINE
  GXInuclNuclei(const LorentzVector<T>& mom,
  ...
  VECCORE_ATT_HOST_DEVICE
  VECCORE_FORCE_INLINE
  GXInuclNuclei(T ekin, Index_v<T> ityp, Model model = DefaultModel) 
  ...
  VECCORE_ATT_HOST_DEVICE
  VECCORE_FORCE_INLINE
  GXInuclNuclei(const GXInuclNuclei<T>& right)
    : BaseClass(right)
    , iType(right.iType)
  { }
  ...
  VECCORE_ATT_HOST_DEVICE
  VECCORE_FORCE_INLINE
  G4InuclNuclei& operator=(const GXInuclNuclei<T>& right)
  {
  ...
  VECCORE_ATT_HOST_DEVICE
  VECCORE_FORCE_INLINE
  vecCore::Mask_v<T> operator==(const GXInuclNuclei<T>& right)
  {
...
static GXParticleDefinition const* getDefinition(int ityp)
{
  ...
  case dineutron:   return G4Dineutron::Definition(); break;
    ...
    */
  default:
    std::cerr << "GXInuclNuclei::getDefinition(): unknown particle type "
	      << ityp << G4endl;
  }
...
template <typename T>
std::ostream &operator<<(std::ostream &os, GXInuclNuclei<T> const& trk)
{
...
template <>
std::ostream &operator<<(std::ostream &os, GXInuclNuclei<double> const& trk)
{
