...
#ifndef GXHadDecayGenerator_HH
#define GXHadDecayGenerator_HH 1
...
public:
  ...
  GXHadDecayGenerator(Algorithm alg=Kopylov, G4int verbose=0);
  GXHadDecayGenerator(GXVHadDecayAlgorithm* alg, G4int verbose=0);
  virtual ~GXHadDecayGenerator();
  ...
  G4bool Generate(const GXParticleDefinition* initialPD,
		  const std::vector<G4double>& masses,
		  std::vector<G4LorentzVector>& finalState);
...
protected:
  ...
  void UseAlgorithm(GXVHadDecayAlgorithm* alg) { theAlgorithm = alg; }

  G4int verboseLevel;
  GXVHadDecayAlgorithm* theAlgorithm;
};

#endif	/* GXHadDecayGenerator_HH */
