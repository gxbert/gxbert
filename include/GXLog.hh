...
inline G4double GXLog(G4double x)
{
        const G4double original_x = x;

        /* separate mantissa from exponent */
        G4double fe;
        x = GXLogConsts::getMantExponent(x,fe);

        // blending
        x > GXLogConsts::SQRTH? fe+=1. : x+=x ;
        x -= 1.0;

        /* rational form */
        G4double px =  GXLogConsts::get_log_px(x);
        ...
        const G4double qx = GXLogConsts::get_log_qx(x);
        ...
        if (original_x > GXLogConsts::LOG_UPPER_LIMIT)
                res = std::numeric_limits<G4double>::infinity();
        if (original_x < GXLogConsts::LOG_LOWER_LIMIT) // THIS IS NAN!
...
inline G4float GXLogf( G4float x )
{
        const G4float original_x = x;

        G4float fe;
        x = GXLogConsts::getMantExponentf( x, fe);

        x > GXLogConsts::SQRTHF? fe+=1.f : x+=x ;
        x -= 1.0f;

        const G4float x2 = x*x;

        G4float res = GXLogConsts::get_log_poly(x);
        res *= x2*x;
        ...
        if (original_x > GXLogConsts::LOGF_UPPER_LIMIT)
                res = std::numeric_limits<G4float>::infinity();
        if (original_x < GXLogConsts::LOGF_LOWER_LIMIT)
...
void GXLogv(const uint32_t size, G4double const * __restrict__ iarray, G4double* __restrict__ oarray);
void logfv(const uint32_t size, G4float const * __restrict__ iarray, G4float* __restrict__ oarray);
void GXLogfv(const uint32_t size, G4float const * __restrict__ iarray, G4float* __restrict__ oarray);
