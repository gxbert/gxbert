...
// ********************************************************************
//
// $Id: GXCascadeInterface.hh 71938 2013-06-28 19:01:00Z mkelsey $
...
#ifndef GXCASCADEINTERFACE_H
#define GXCASCADEINTERFACE_H 1

//#include "G4VIntraNuclearTransportModel.hh"
//#include "GXFragmentVector.hh"
...
//class G4V3DNucleus; //Not really necessary

struct GXNucleus {
   int A;
   int Z;

   GXNucleus( G4int a , G4int z ) { Z = z; A = a;};
   G4int GetA_asInt(){return A;}
   G4int GetZ_asInt(){return Z;}
};

struct GXHadProjectile{
   const GXParticleDefinition* particleDefinition;
   G4double kineticEnergy;
   G4LorentzVector fourMomentum; 

   GXHadProjectile( const GXParticleDefinition* pd , G4double ke ){ particleDefinition=pd; kineticEnergy=ke; G4double e,p; e=ke+pd->GetPDGMass(); p=std::sqrt(e*e-pd->GetPDGMass()*pd->GetPDGMass());fourMomentum.setX(0); fourMomentum.setY(0); fourMomentum.setZ(p); fourMomentum.setT(e); }
   const GXParticleDefinition* GetDefinition() const {return particleDefinition;}
   G4double GetKineticEnergy() const {return kineticEnergy;}
   G4LorentzVector Get4Momentum() const {return fourMomentum;}
};

enum GXHadFinalStateStatus{isAlive, stopAndKill, suspend};
struct GXHadFinalState{
   GXHadFinalStateStatus theStatus;
   std::vector<GXDynamicParticle*> theSecondaries;
   void SetStatusChange(GXHadFinalStateStatus aS){ theStatus=aS; }
   void SetEnergyChange(G4double){;}
   void Clear(){ for ( auto it = theSecondaries.begin() ; it != theSecondaries.end() ; it++ ) delete *it; theSecondaries.clear();}
   void AddSecondary(GXDynamicParticle* dp){theSecondaries.push_back(dp);}
   std::vector<GXDynamicParticle*> GetSecondaries(){return theSecondaries;};
};

class GXCascadeInterface {

public:
  GXCascadeInterface(const G4String& name = "BertiniCascade");

  virtual ~GXCascadeInterface();

  GXHadFinalState* ApplyYourself(const GXHadProjectile& aTrack,
				 GXNucleus& theNucleus);

  void SetVerboseLevel(G4int verbose);		// Overrides base class

  G4bool IsApplicable(const GXHadProjectile& aTrack,
		      GXNucleus& theNucleus);

  G4bool IsApplicable(const GXParticleDefinition* aPD) const;
...
protected:
  ...
  G4bool createBullet(const GXHadProjectile& aTrack);
  G4bool createTarget(GXNucleus& theNucleus);
//  G4bool createTarget(G4V3DNucleus* theNucleus);
  ...
  GXHadFinalState* NoInteraction(const GXHadProjectile& aTrack,
				 GXNucleus& theNucleus);
  ...
  GXDynamicParticle* makeDynamicParticle(const G4InuclElementaryParticle& iep) const;
  GXDynamicParticle* makeDynamicParticle(const G4InuclNuclei& inuc) const;


private:
  G4int operator==(const GXCascadeInterface& right) const {
    return (this == &right);
  }
  ...
  //G4ParticleDefinition* convertGXPD2G4PD( const GXParticleDefinition* ) const;
  //GXParticleDefinition* convertG4PD2GXPD( const G4ParticleDefinition* ) const;
  //G4DynamicParticle convertGXDP2G4DP( const GXDynamicParticle& ) const;
  //GXDynamicParticle* convertG4DP2GXDP( const G4DynamicParticle* ) const;

private:
  // Copying of modules is forbidden
  GXCascadeInterface(const GXCascadeInterface&);
  GXCascadeInterface& operator=(const GXCascadeInterface&);

private:
  //TK addin dummy verboseLevel for GXBERT
  G4int verboseLevel;
  GXHadFinalState theParticleChange;
};

#endif // GXCASCADEINTERFACE_H
