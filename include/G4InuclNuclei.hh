...
public:
  ...
  G4InuclNuclei(const GXDynamicParticle& dynPart, Model model=DefaultModel)
  ...
  G4InuclNuclei(const GXFragment& aFragment, Model model=DefaultModel);
  ...
  void copy(const GXFragment& aFragment, Model model=DefaultModel);
  ...
  GXFragment makeGXFragment() const;
  operator GXFragment() const;

protected:
  // Convert nuclear configuration to standard GEANT4 pointer
  static GXParticleDefinition* makeDefinition(G4int a, G4int z);
  static GXParticleDefinition* makeNuclearFragment(G4int a, G4int z);
