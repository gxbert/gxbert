...
// Code and algorithms for GXExp have been extracted and adapted for Geant4
...
inline G4double GXExp(G4double initial_x)
{
    G4double x = initial_x;
    G4double px=GXExpConsts::fpfloor(GXExpConsts::LOG2E * x +0.5);
    ...
    // px = x * P(x**2).
    px = GXExpConsts::PX1exp;
    px *= xx;
    px += GXExpConsts::PX2exp;
    px *= xx;
    px += GXExpConsts::PX3exp;
    px *= x;

    // Evaluate Q(x**2).
    G4double qx = GXExpConsts::QX1exp;
    qx *= xx;
    qx += GXExpConsts::QX2exp;
    qx *= xx;
    qx += GXExpConsts::QX3exp;
    qx *= xx;
    qx += GXExpConsts::QX4exp;
    ...
    // Build 2^n in double.
    x *= GXExpConsts::uint642dp(( ((uint64_t)n) +1023)<<52);

    if (initial_x > GXExpConsts::EXP_LIMIT)
...
inline G4float GXExpf(G4float initial_x)
{
    G4float x = initial_x;

    G4float z = GXExpConsts::fpfloor( GXExpConsts::LOG2EF * x +0.5f ); /* std::floor() truncates toward -infinity. */
    ...
    z = x*GXExpConsts::PX1expf;
    z += GXExpConsts::PX2expf;
    z *= x;
    z += GXExpConsts::PX3expf;
    z *= x;
    z += GXExpConsts::PX4expf;
    z *= x;
    z += GXExpConsts::PX5expf;
    z *= x;
    z += GXExpConsts::PX6expf;
    z *= x2;
    z += x + 1.0f;

    /* multiply by power of 2 */
    z *=  GXExpConsts::uint322sp((n+0x7f)<<23);

    if (initial_x > GXExpConsts::MAXLOGF) z=std::numeric_limits<G4float>::infinity();
    if (initial_x < GXExpConsts::MINLOGF) z=0.f;
...
void GXExpv(const uint32_t size, G4double const * __restrict__ iarray, G4double* __restrict__ oarray);
void expfv(const uint32_t size, G4float const * __restrict__ iarray, G4float* __restrict__ oarray);
void GXExpfv(const uint32_t size, G4float const * __restrict__ iarray, G4float* __restrict__ oarray);
