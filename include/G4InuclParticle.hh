...
// 20100519  M. Kelsey -- Add public access to GXDynamicParticle content
// 20100715  M. Kelsey -- Add setKineticEnergy() function
// 20100915  M. Kelsey -- Add constructor to copy GXDynamicParticle input
...
public:
  ...
  G4InuclParticle(const GXDynamicParticle& dynPart, Model model=DefaultModel)
  ...
  // These are call-throughs to GXDynamicParticle
  ...
  const GXParticleDefinition* getDefinition() const {
    return pDP.GetDefinition();
  }

  const GXDynamicParticle& getDynamicParticle() const {
    return pDP;
  }
...
protected: 
  ...
  G4InuclParticle(const GXParticleDefinition* pd, const G4LorentzVector& mom,
		  Model model=DefaultModel);

  // NOTE:  Momentum forced along Z direction
  G4InuclParticle(const GXParticleDefinition* pd, G4double ekin,
		  Model model=DefaultModel)
    : pDP(pd,G4ThreeVector(0.,0.,1.),ekin*CLHEP::GeV/CLHEP::MeV), modelId(model) {}

  void setDefinition(const GXParticleDefinition* pd);

private:
  GXDynamicParticle pDP;		// Carries all the kinematics and info
  Model modelId;
};        
