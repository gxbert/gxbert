...
protected:
  ...
  virtual G4bool explosion(const GXFragment& target) const;
...
protected:
  ...
  virtual G4bool validateOutput(const GXFragment& target,
  ...
  void getTargetData(const GXFragment& target);
  ...
  const GXFragment& makeFragment(G4LorentzVector mom, G4int A, G4int Z,
				 G4double EX=0.);
  const GXFragment& makeFragment(G4int A, G4int Z, G4double EX=0.);
  GXFragment aFragment;	// Reusable buffer to reduce new/delete cycling
