...
void G4CascadeDeexciteBase::getTargetData(const GXFragment& target) {
  A = target.GetA_asInt();
  Z = target.GetZ_asInt();
...
G4bool G4CascadeDeexciteBase::explosion(const GXFragment& fragment) const {
...
G4bool G4CascadeDeexciteBase::validateOutput(const GXFragment& target,
