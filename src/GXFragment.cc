...
G4ThreadLocal GXAllocator<GXFragment> *pGXFragmentAllocator = nullptr;
const G4double GXFragment::minFragExcitation = 10.*CLHEP::eV;

// Default constructor
GXFragment::GXFragment() :
  theA(0),
  theZ(0),
...
// Copy Constructor
GXFragment::GXFragment(const GXFragment &right) :
   theA(right.theA),
   theZ(right.theZ),
...
GXFragment::GXFragment(G4int A, G4int Z, const G4LorentzVector& aMomentum) :
  theA(A),
  theZ(Z),
  theExcitationEnergy(0.0),
  theGroundStateMass(0.0),
...
/*
GXFragment::GXFragment(const G4LorentzVector& aMomentum, 
...
//  theCreationTime(0.0)
{
  if(aParticleDefinition->GetPDGEncoding() != 22 && 
    ...
    G4String text = "GXFragment::GXFragment constructor for gamma used for "
      + aParticleDefinition->GetParticleName();  
    throw GXHadronicException(__FILE__, __LINE__, text);
  }
  theGroundStateMass = aParticleDefinition->GetPDGMass();
}
*/

GXFragment & GXFragment::operator=(const GXFragment &right)
{
  if (this != &right) {
    theA = right.theA;
    theZ = right.theZ;
...
G4bool GXFragment::operator==(const GXFragment &right) const
{
  return (this == (GXFragment *) &right);
}
...
std::ostream& operator << (std::ostream &out, const GXFragment &theFragment)
{
...
void GXFragment::ExcitationEnergyWarning()
{
#ifdef G4VERBOSE
  G4cout << "GXFragment::CalculateExcitationEnergy(): WARNING "<<G4endl;
  G4cout << *this << G4endl;
#endif
}

void GXFragment::NumberOfExitationWarning(const G4String& value)
{
  G4cout << "GXFragment::"<< value << " ERROR "
	 << G4endl;
  G4cout << this << G4endl; 
  G4String text = "GXFragment::GXFragment wrong exciton number ";
  throw GXHadronicException(__FILE__, __LINE__, text);
}

/*
void GXFragment::SetAngularMomentum(const G4ThreeVector& v)
{
  spin = v.mag();
}

G4ThreeVector GXFragment::GetAngularMomentum() const
{
