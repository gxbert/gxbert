...
//
//
// $Id: GXParticleDefinition.cc 106143 2017-09-14 06:34:42Z gcosmo $
//
// 
// --------------------------------------------------------------
...
//
//TK for GXBERT
//G4PDefManager GXParticleDefinition::subInstanceManager;
...
GXParticleDefinition::GXParticleDefinition(
		   ...
		   isMuonicAtom(false)
{
   ...
   theParticleTable = GXParticleTable::GetParticleTable();
...
/*
   ...
   if ( !fShortLivedFlag && (theParticleType!=nucleus) && (theParticleType!=muAtom) && (currentState!=G4State_PreInit)){
     ...
     G4Exception( "G4ParticleDefintion::G4ParticleDefintion",
		  "PART101", JustWarning, 
		  "GXParticleDefinition should be created in PreInit state");
   }
*/
...
GXParticleDefinition::GXParticleDefinition(const GXParticleDefinition &)
{
  G4Exception("GXParticleDefinition::GXParticleDefinition()",
	      "PART001", FatalException,
	      "Illegal call of copy Constructor for GXParticleDefinition ");
}
...
GXParticleDefinition::GXParticleDefinition()
{
  G4Exception("GXParticleDefinition::GXParticleDefinition()",
	      "PART001", FatalException,
	      "Illegal call of default Constructor for GXParticleDefinition ");
}
...
const GXParticleDefinition & GXParticleDefinition::operator=(const GXParticleDefinition &right)
{
  if (this != &right)  {
  } 
  return *this;
...
G4int GXParticleDefinition::operator==(const GXParticleDefinition &right) const
{
...
//TK for GXBERT
/*
const G4PDefManager& GXParticleDefinition::GetSubInstanceManager()
...
void GXParticleDefinition::Clean()
...
G4ProcessManager* GXParticleDefinition::GetProcessManager() const
{
    //TK for GXBERTT
    return NULL;
...
G4int GXParticleDefinition::FillQuarkContents()
...
#ifdef G4VERBOSE
	if (verboseLevel>0) {
	  G4cout << "GXParticleDefinition::FillQuarkContents  : "
...
void GXParticleDefinition::DumpTable() const
{
  G4cout << G4endl;
  G4cout << "--- GXParticleDefinition ---" << G4endl;
  ...
  if ( IsGeneralIon() ) {
    ...
    else
    {
      ...
      G4cout << " Stable : unstable -- lifetime = " << lftm/CLHEP::s << "[s]"
...
void GXParticleDefinition::SetApplyCutsFlag(G4bool flg)
{
  ...
  {
    G4cout
     << "GXParticleDefinition::SetApplyCutsFlag() for " << theParticleName
     << G4endl;
    G4cout
...
G4double GXParticleDefinition::CalculateAnomaly()  const
{
  G4Exception( "G4ParticleDefintion::G4ParticleDefintion",
               "PART114", JustWarning, 
...
/*
void GXParticleDefinition::SetParticleDefinitionID(G4int id)
{
  if(id<0)
  {
...
void GXParticleDefinition::SetProcessManager(G4ProcessManager*  /* aProcessManager */)
{
  //TK for GXBERT
  //Process manager is not needed in GXParticleDefinition

  //if(g4particleDefinitionInstanceID<0 && !isGeneralIon)
  //{
    //TK for GXBERT: Exclude to use G4Threading
