...
// 20130924  M. Kelsey -- Use GXLog, GXExp, GXPow for CPU speedup
// 20150619  M. Kelsey -- Define G4cbrt(int) to use GXPow::Z13, rearrange
//		FermiEnergy() to use GXPow::Z23.
...
G4InuclSpecialFunctions::randomInuclPowers(G4double ekin, 
  ...
  GXPow* theGXPow = GXPow::GetInstance();

  G4double S = inuclRndm();		// Random fraction for expansion
  ...
  for (G4int i=0; i<4; i++) {
    ...
    for (G4int k=0; k<4; k++) {
      C = coeff[i][k];
      V += C * theGXPow->powN(ekin, k);
    }

    PQ += V;
    PR += V * theGXPow->powN(S, i);
  }
...
G4double G4InuclSpecialFunctions::G4cbrt(G4double x) {
  return x==0 ? 0. : (x<0?-1.:1.)*GXExp(GXLog(std::fabs(x))/3.);
}

G4double G4InuclSpecialFunctions::G4cbrt(G4int n) {
  return n==0 ? 0. : (n<0?-1.:1.)*GXPow::GetInstance()->Z13(std::abs(n));
}
