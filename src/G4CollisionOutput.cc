...
typedef std::vector<GXFragment>::iterator fragmentIterator;

const GXFragment G4CollisionOutput::emptyFragment;		// All zeros
...
const GXFragment& G4CollisionOutput::getRecoilFragment(G4int index) const {
...
//G4cout << "G4CollisionOutput::addOutgoingParticles(const G4ReactionProductVector* rproducts) is unexpectedly called. Abort this job!" << G4endl, abort();
/*
  ...
  for (j=rproducts->begin(); j!=rproducts->end(); ++j) {
    const GXParticleDefinition* pd = (*j)->GetDefinition();
