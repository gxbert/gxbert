...
void G4Fissioner::deExcite(const GXFragment& target,
...
void G4Fissioner::potentialMinimization(G4double& VP, 
  ...
  while (itry < itry_max) {	/* Loop checking 08.06.2015 MHK */
	...
	if (std::fabs(AL1[i]) >= DS1) {
	  ...
	  G4double DEX = XXX > 100.0 ? huge_num : GXExp(XXX);
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (std::fabs(AL1[i]) >= DS1) DX1 = 2.0 * AL1[i] * DS2 * GXExp(AL1[i] * AL1[i] * DS2);

      if (std::fabs(BET1[i]) >= DS1) DX2 = 2.0 * BET1[i] * DS2 * GXExp(BET1[i] * BET1[i] * DS2);
