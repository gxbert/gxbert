...
//
//
// $Id: GXPhaseSpaceDecayChannel.cc 102916 2017-03-02 12:58:58Z gcosmo $
//
// 
// ------------------------------------------------------------
...
//G4ThreadLocal G4double GXPhaseSpaceDecayChannel::current_parent_mass = 0.0;

GXPhaseSpaceDecayChannel::GXPhaseSpaceDecayChannel(G4int Verbose)
  :GXVDecayChannel("Phase Space", Verbose),
   useGivenDaughterMass(false)
{

}

GXPhaseSpaceDecayChannel::GXPhaseSpaceDecayChannel(
...
GXDecayProducts *GXPhaseSpaceDecayChannel::DecayIt(G4double parentMass)
{
#ifdef G4VERBOSE
  if (GetVerboseLevel()>1) G4cout << "GXPhaseSpaceDecayChannel::DecayIt ";
#endif
  
  GXDecayProducts * products = 0;
  ...
  case 0:
    ...
    if (GetVerboseLevel()>0) {
      G4cout << "GXPhaseSpaceDecayChannel::DecayIt ";
...
GXDecayProducts *GXPhaseSpaceDecayChannel::OneBodyDecayIt()
{
#ifdef G4VERBOSE
  if (GetVerboseLevel()>1) G4cout << "GXPhaseSpaceDecayChannel::OneBodyDecayIt()"<<G4endl;
#endif
  // parent mass
  //TK for GXBERT
  ...
  //create parent GXDynamicParticle at rest
  G4ThreeVector dummy;
  GXDynamicParticle * parentparticle = new GXDynamicParticle( G4MT_parent, dummy, 0.0, parentmass);
  //create G4Decayproducts
  GXDecayProducts *products = new GXDecayProducts(*parentparticle);
  delete parentparticle;

  //create daughter GXDynamicParticle at rest
  GXDynamicParticle * daughterparticle = new GXDynamicParticle( G4MT_daughters[0], dummy, 0.0);
  if (useGivenDaughterMass) daughterparticle->SetMass(givenDaughterMasses[0]);
  products->PushProducts(daughterparticle);

#ifdef G4VERBOSE
  if (GetVerboseLevel()>1) {
     G4cout << "GXPhaseSpaceDecayChannel::OneBodyDecayIt ";
...
GXDecayProducts *GXPhaseSpaceDecayChannel::TwoBodyDecayIt()
{
#ifdef G4VERBOSE
  if (GetVerboseLevel()>1) G4cout << "GXPhaseSpaceDecayChannel::TwoBodyDecayIt()"<<G4endl;
#endif
  // parent mass
  //TK for GXBERT
  //G4double parentmass = current_parent_mass.Get();
...
#ifdef G4VERBOSE
	if (GetVerboseLevel()>0) {
	  G4cout << "GXPhaseSpaceDecayChannel::TwoBodyDecayIt " 
...
#endif
	G4Exception("GXPhaseSpaceDecayChannel::TwoBodyDecayIt",
		    "PART112", JustWarning,
  ...
  //create daughter GXDynamicParticle 
  G4double Ekin = std::sqrt(daughtermomentum*daughtermomentum + daughtermass[0]*daughtermass[0]) - daughtermass[0];
  GXDynamicParticle * daughterparticle = new GXDynamicParticle( G4MT_daughters[0], direction, Ekin, daughtermass[0]);
  products->PushProducts(daughterparticle);
  Ekin = std::sqrt(daughtermomentum*daughtermomentum + daughtermass[1]*daughtermass[1]) - daughtermass[1];
  daughterparticle = new GXDynamicParticle( G4MT_daughters[1], -1.0*direction, Ekin, daughtermass[1]);
  products->PushProducts(daughterparticle);
...
GXDecayProducts *GXPhaseSpaceDecayChannel::ThreeBodyDecayIt()
// algorism of this code is originally written in GDECA3 of GEANT3
{
#ifdef G4VERBOSE
  if (GetVerboseLevel()>1) G4cout << "GXPhaseSpaceDecayChannel::ThreeBodyDecayIt()"<<G4endl;
#endif
  // parent mass
  //TK for GXBERT
  //G4double parentmass = current_parent_mass.Get();
...
#ifdef G4VERBOSE
	if (GetVerboseLevel()>0) {
	  G4cout << "GXPhaseSpaceDecayChannel::ThreeBodyDecayIt " 
		 << "sum of daughter mass is larger than parent mass" << G4endl;
          //TK for GXBERT
	  //G4cout << "parent :" << G4MT_parent->GetParticleName() << "  " << current_parent_mass.Get()/GeV << G4endl;
	  G4cout << "parent :" << G4MT_parent->GetParticleName() << "  " << current_parent_mass/GeV << G4endl;
...
#endif
	G4Exception("GXPhaseSpaceDecayChannel::ThreeBodyDecayIt",
		    "PART112", JustWarning,
		    "Can not create decay products: sum of daughter mass is larger than parent mass");
	return products;
      }
  ...
  GXDynamicParticle * daughterparticle = new GXDynamicParticle( G4MT_daughters[0], 
								direction0, 
								Ekin, daughtermass[0]);
  products->PushProducts(daughterparticle);
  ...
  daughterparticle = new GXDynamicParticle( G4MT_daughters[2], 
					    pmom/pmom.mag(),
					    Ekin, daughtermass[2]);
  products->PushProducts(daughterparticle);
  ...
  daughterparticle = new GXDynamicParticle( 
					   G4MT_daughters[1],
					   pmom/pmom.mag(),
					   Ekin, daughtermass[1]);
  products->PushProducts(daughterparticle);
...
GXDecayProducts *GXPhaseSpaceDecayChannel::ManyBodyDecayIt()
...
//
{
...
#ifdef G4VERBOSE
  if (GetVerboseLevel()>1) G4cout << "GXPhaseSpaceDecayChannel::ManyBodyDecayIt()"<<G4endl;
#endif
...
#ifdef G4VERBOSE
    if (GetVerboseLevel()>0) {
      G4cout << "GXPhaseSpaceDecayChannel::ManyBodyDecayIt " 
	     << "sum of daughter mass is larger than parent mass" << G4endl;
      //TK for GXBERT
      //G4cout << "parent :" << G4MT_parent->GetParticleName() << "  " << current_parent_mass.Get()/GeV << G4endl;
      G4cout << "parent :" << G4MT_parent->GetParticleName() << "  " << current_parent_mass/GeV << G4endl;
...
#endif
    G4Exception("GXPhaseSpaceDecayChannel::ManyBodyDecayIt",
		"PART112", JustWarning,
		"Can not create decay products: sum of daughter mass is larger than parent mass");
    delete [] daughtermass;
    return products;
  ...
  GXDynamicParticle **daughterparticle;
  ...
  G4double beta;
  daughterparticle = new GXDynamicParticle*[numberOfDaughters];
  ...
  daughterparticle[index] = new GXDynamicParticle( G4MT_daughters[index], direction*daughtermomentum[index] );
  daughterparticle[index+1] = new GXDynamicParticle( G4MT_daughters[index+1], direction*(-1.0*daughtermomentum[index]) );
  ...
  for (index = numberOfDaughters -3;  index >= 0; index--) {
    ...
    //create daughter GXDynamicParticle 
    daughterparticle[index]= new GXDynamicParticle( G4MT_daughters[index], direction*(-1.0*daughtermomentum[index]));
  }
...
G4bool  GXPhaseSpaceDecayChannel::SetDaughterMasses( G4double masses[])
{
...
G4bool  GXPhaseSpaceDecayChannel::SampleDaughterMasses()
{
  useGivenDaughterMass = false;
  return useGivenDaughterMass; 
}

G4bool  GXPhaseSpaceDecayChannel::IsOKWithParentMass(G4double parentMass) {
  if (!useGivenDaughterMass) return GXVDecayChannel::IsOKWithParentMass(parentMass);
...
G4double GXPhaseSpaceDecayChannel::Pmx(G4double e, G4double p1, G4double p2)
{
