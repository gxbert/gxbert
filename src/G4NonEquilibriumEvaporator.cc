...
// 20130924  M. Kelsey -- Replace std::pow with GXPow::powN() for CPU speed
...
G4NonEquilibriumEvaporator::G4NonEquilibriumEvaporator()
  : G4CascadeDeexciteBase("G4NonEquilibriumEvaporator"),
    theParaMaker(verboseLevel), theGXPow(GXPow::GetInstance()) {}


void G4NonEquilibriumEvaporator::deExcite(const GXFragment& target,
	...
	if (EMN > eexs_cut) { // ok
	  ...
	  if (NEX > 1) {
	    ...
	    if (F1 > 0.0 && F2 > 0.0) {
	      ...
	      G4double D[3] = { 0., 0., 0. };
	      D[0] = M1 * F2 * F2 * theGXPow->powN(F, NEX-1) / (QEX+1);
	      if (verboseLevel > 3) {
		G4cout << " D[0] " << D[0] << " with F " << F
		       << " powN(F,NEX-1) " << theGXPow->powN(F, NEX-1)
		       << G4endl;
	      }
		...
		if (NEX >= 2) {
		  ...
		  if (EMP > eexs_cut) 
		    D[2] = D[1] * theGXPow->powN(EMP/EEXS, NEX) * (1.0 + CPA1);
		  D[1] *= theGXPow->powN(EMN/EEXS, NEX) * getAL(A);   

		  if (verboseLevel > 3) {
		    G4cout << " D[1] " << D[1] << " with powN(EMN/EEXS, NEX) "
			   << theGXPow->powN(EMN/EEXS, NEX) << G4endl
			   << " D[2] " << D[2] << " with powN(EMP/EEXS, NEX) "
			   << theGXPow->powN(EMP/EEXS, NEX) << G4endl;
		  }
		  ...
		  if (E < 0.0) icase = 0;
			...
			if (NEX == 2) {
			  ...
			  for (G4int i = 0; i < 1000; i++) {
			    G4double DX = X * QEX1 * 
			      (1.0 + QEX2 * X * (1.0 - R / theGXPow->powN(X, NEX)) / (1.0 - X));
			    if (verboseLevel > 3) {
			      G4cout << " NEX " << NEX << " powN(X, NEX) "
				     << theGXPow->powN(X, NEX) << G4endl;
			    }
