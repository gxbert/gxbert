...
//
//
// $Id: GXVDecayChannel.cc 105720 2017-08-16 12:38:10Z gcosmo $
//
// 
// ------------------------------------------------------------
...
const G4String GXVDecayChannel::noName = " ";

GXVDecayChannel::GXVDecayChannel()
  :kinematics_name(""),
   rbranch(0.0),
   numberOfDaughters(0),
   parent_name(nullptr), 
   ...
   verboseLevel(1)		
{
  ...
  // set pointer to GXParticleTable (static and singleton object)
  particletable = GXParticleTable::GetParticleTable();
}

GXVDecayChannel::GXVDecayChannel(const G4String &aName, G4int Verbose)
...
GXVDecayChannel::GXVDecayChannel(const G4String  &aName, 
...
GXVDecayChannel::GXVDecayChannel(const GXVDecayChannel &right)
{
...
GXVDecayChannel & GXVDecayChannel::operator=(const GXVDecayChannel &right)
{
  if (this != &right) { 
    kinematics_name = right.kinematics_name;
    verboseLevel = right.verboseLevel;
...
void GXVDecayChannel::ClearDaughtersName()
{   
  //TK for GXBERT: Exclude to use G4AutoLock
...
#ifdef G4VERBOSE
      if (verboseLevel>1) {
	G4cout << "GXVDecayChannel::ClearDaughtersName "
...
void GXVDecayChannel::SetNumberOfDaughters(G4int size)
{
  if (size >0) {
    // remove old contents
    ClearDaughtersName();
...
void GXVDecayChannel::SetDaughter(G4int anIndex, 
				 const G4String &particle_name)
{
...
#ifdef G4VERBOSE
    if (verboseLevel>0) {
      G4cout << "GXVDecayChannel::SetDaughter: "
  ...
  if ( daughters_name == nullptr ) {
    G4Exception("GXVDecayChannel::SetDaughter","PART112",FatalException,
		"Trying to add a daughter without specifying number of secondaries, useSetNumberOfDaughters first");
    return;
  }
  if ( G4MT_daughters != nullptr ) {
    G4Exception("GXVDecayChannel::SetDaughter","PART111",FatalException,
...
#ifdef G4VERBOSE
    if (verboseLevel>1) {
      G4cout << "GXVDecayChannel::SetDaughter[" << anIndex <<"] :";
...
void GXVDecayChannel::SetDaughter(G4int anIndex, const GXParticleDefinition * parent_type)
{
  if (parent_type != nullptr) SetDaughter(anIndex, parent_type->GetParticleName());
}

void GXVDecayChannel::FillDaughters()
{
  //TK for GXBERT: Exclude to use G4AutoLock
...
#ifdef G4VERBOSE
  if (verboseLevel>1) G4cout << "GXVDecayChannel::FillDaughters()" <<G4endl;
#endif
  if (G4MT_daughters != nullptr) {
...
#ifdef G4VERBOSE
    if (verboseLevel>0) {
      G4cout << "GXVDecayChannel::FillDaughters   "
             << "[ " << G4MT_parent->GetParticleName() << " ]"
             << "numberOfDaughters is not defined yet";
    }
#endif
    G4MT_daughters = nullptr;
    G4Exception("GXVDecayChannel::FillDaughters",
  ...
  G4MT_daughters = new GXParticleDefinition*[numberOfDaughters];
  if (G4MT_daughters_mass != nullptr) delete [] G4MT_daughters_mass;
  if (G4MT_daughters_width != nullptr) delete [] G4MT_daughters_width;
...
void GXVDecayChannel::FillParent()
{
  //TK for GXBERT: Exclude to use G4AutoLock
...
#ifdef G4VERBOSE
    if (verboseLevel>0) {
      G4cout << "GXVDecayChannel::FillParent   "
             << ": parent name is not defined !!" << G4endl;
    }
#endif
    G4MT_parent = nullptr;
    G4Exception("GXVDecayChannel::FillParent()",
...
void GXVDecayChannel::SetParent(const GXParticleDefinition * parent_type)
{
  if (parent_type != nullptr) SetParent(parent_type->GetParticleName());
}

G4int GXVDecayChannel::GetAngularMomentum()
{
  ...
  if (2==numberOfDaughters) {     // up to now we can only handle two particle decays
    ...
    G4Exception("GXVDecayChannel::GetAngularMomentum",
		"PART111", JustWarning,
...
const G4String& GXVDecayChannel::GetNoName() const
{
  return noName;
}

#include "Randomize.hh"
G4double GXVDecayChannel::DynamicalMass(G4double massPDG, G4double width, G4double maxDev ) const
{ 
...
G4bool    GXVDecayChannel::IsOKWithParentMass(G4double parentMass)
{
...
void  GXVDecayChannel::SetBR(G4double value)
{ 
  rbranch = value; 
  if      (rbranch <0.  ) rbranch = 0.0;
  else if (rbranch >1.0 ) rbranch = 1.0;
