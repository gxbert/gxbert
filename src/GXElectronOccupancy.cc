...
//
//
// $Id: GXElectronOccupancy.cc 79357 2014-02-25 10:06:54Z gcosmo $
//
// 
// ----------------------------------------------------------------------
...
G4ThreadLocal GXAllocator<GXElectronOccupancy> *aGXElectronOccupancyAllocator = 0;

GXElectronOccupancy::GXElectronOccupancy(G4int sizeOrbit )
...
GXElectronOccupancy::~GXElectronOccupancy()
{
   theSizeOfOrbit = -1;
...
GXElectronOccupancy::GXElectronOccupancy(const GXElectronOccupancy& right)
{
...
GXElectronOccupancy& GXElectronOccupancy::operator=(const GXElectronOccupancy& right)
{
  if ( this != &right) {
    theSizeOfOrbit = right.theSizeOfOrbit;
...
G4int GXElectronOccupancy::operator==(const GXElectronOccupancy& right) const
{
  G4int index;
  G4bool value = true;
...
void GXElectronOccupancy::DumpInfo() const
{
...
G4int  GXElectronOccupancy::AddElectron(G4int orbit, G4int number)
{
  G4int value =0;
  if (orbit>=theSizeOfOrbit){
    ...
    G4Exception("GXElectronOccupancy::AddElectron()","PART131",
...
G4int  GXElectronOccupancy::RemoveElectron(G4int orbit, G4int number)
{
  G4int value =0;
  if (orbit>=theSizeOfOrbit){
    std::ostringstream smsg;
	...
	<<theSizeOfOrbit-1 <<") ";
    G4String msg = smsg.str();
    G4Exception("GXElectronOccupancy::RemoveElectron()","PART131",
		JustWarning, msg.c_str());
  } else if (orbit >=0) {
