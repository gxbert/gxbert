...
//		thread collisions
// 20130924  M. Kelsey -- Use GXLog, GXExp, G4Pow for CPU speedup
...
void G4NucleiModel::fillZoneRadii(G4double nuclearRadius) {
  ...
  G4double skinDecay = GXExp(-skinRatio);    
  ...
  if (A < 5) {			// Light ions treated as simple balls
    ...
    for (G4int i = 0; i < number_of_zones; i++) {
      G4double y = std::sqrt(-GXLog(alfa3[i]));
    ...
    ur[0] = -skinRatio;
    for (G4int i = 0; i < number_of_zones; i++) {
      G4double y = GXLog((1.0 + skinDecay)/alfa3[i] - 1.0);
    ...
    ur[0] = -skinRatio;
    for (G4int i = 0; i < number_of_zones; i++) {
      G4double y = GXLog((1.0 + skinDecay)/alfa6[i] - 1.0);
      zone_radii.push_back(nuclearRadius + skinDepth * y);
      ur[i+1] = y;
    }
  }
...
G4double G4NucleiModel::zoneIntegralWoodsSaxon(G4double r1, G4double r2, 
  ...
  G4double dr = r2 - r1;
  G4double fr1 = r1 * (r1 + d2) / (1.0 + GXExp(r1));
  G4double fr2 = r2 * (r2 + d2) / (1.0 + GXExp(r2));
  ...
  return skinDepth3 * (fun + skinRatio*skinRatio*GXLog((1.0 + GXExp(-r1)) / (1.0 + GXExp(-r2))));
}
...
G4double G4NucleiModel::zoneIntegralGaussian(G4double r1, G4double r2, 
  ...
  G4double dr = r2 - r1;
  G4double fr1 = r1 * r1 * GXExp(-r1 * r1);
  G4double fr2 = r2 * r2 * GXExp(-r2 * r2);
  G4double fi = (fr1 + fr2) / 2.;
  G4double fun1 = fi * dr;
  G4double fun;
  G4int jc = 1;
...
void G4NucleiModel::choosePointAlongTraj(G4CascadParticle& cparticle) {
  ...
  for (i=1; i<ncross; i++) {
    ...
    G4double wt = (GXExp(-len[i-1]*invmfp)-GXExp(-len[i]*invmfp)) / invmfp;
...
void G4NucleiModel::initializeCascad(G4InuclNuclei* bullet, 
  ...
  if (ab < max_a_for_cascad) {
	...
	if (ab < 3) { // deuteron, simplest case
	  G4double r = 2.214 - 3.4208 * GXLog(1.0 - 0.981 * inuclRndm());
	  ...
	  if (ab == 3) {
		...
		G4int itry1 = 0;
		G4double ss, u, rho; 
		G4double fmax = GXExp(-0.5) / std::sqrt(0.5);

		while (itry1 < itry_max) {  /* Loop checking 08.06.2015 MHK */
		  itry1++;
		  ss = -GXLog(inuclRndm());
		  u = fmax * inuclRndm();
		  rho = std::sqrt(ss) * GXExp(-ss);
	    ...
	    G4double fmax = (1.0 + u/b) * u * GXExp(-u);
		...
		while (itry1 < itry_max) {  /* Loop checking 08.06.2015 MHK */
		  ...
		  if (std::sqrt(ss) * GXExp(-ss) * (1.0 + ss/b) > u
		      && ss < s4max) {
	  ...
	  if(badco) {
		...
		itry2++;
		u = -GXLog(0.879853 - 0.8798502 * inuclRndm());
		x = u * GXExp(-u);
...
G4NucleiModel::generateInteractionLength(const G4CascadParticle& cparticle,
  ...
  if (forceFirst(cparticle) || (inuclRndm() < pw)) {
    spath = -GXLog(1.0 - pw * inuclRndm()) / invmfp;
