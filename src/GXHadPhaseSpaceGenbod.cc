...
GXHadPhaseSpaceGenbod::GXHadPhaseSpaceGenbod(G4int verbose)
  : GXVHadPhaseSpaceAlgorithm("GXHadPhaseSpaceGenbod",verbose),
    nFinal(0), totalMass(0.), massExcess(0.), weightMax(0.), nTrials(0) {;}


// C++ re-implementation of GENBOD.F (Raubold-Lynch method)

void GXHadPhaseSpaceGenbod::
GenerateMultiBody(G4double initialMass,
  ...
  if ( nTrials >= maxNumberOfLoops ) {
    ...
    G4Exception( " GXHadPhaseSpaceGenbod::GenerateMultiBody ", "HAD_GENBOD_001", FatalException, ed );
  }
...
void GXHadPhaseSpaceGenbod::FillRandomBuffer() {
...
void 
GXHadPhaseSpaceGenbod::FillEnergySteps(G4double initialMass,
				       const std::vector<G4double>& masses) {
...
void 
GXHadPhaseSpaceGenbod::ComputeWeightScale(const std::vector<G4double>& masses) {
...
G4double GXHadPhaseSpaceGenbod::ComputeWeight() const {
...
G4bool GXHadPhaseSpaceGenbod::AcceptEvent() const {
  if (GetVerboseLevel()>1) 
