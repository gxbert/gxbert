...
// 20130924  M. Kelsey -- Use GXLog, GXExp for CPU speedup
...
void G4EquilibriumEvaporator::deExcite(const GXFragment& target,
  ...
  while (try_again && itry_global < itry_global_max) {
    ...
    if (prob_sum < prob_cut_off) { 		// photon emission chain
      ...
      while (EEXS > cut_off_energy && try_again) {
        ...
        if (T04 < EEXS) {
          FMAX = (T04*T04*T04*T04) * GXExp((EEXS - T04) / T00);
        } else {
      ...
      if (icase < 6) { // particle or light nuclei escape
        ...
        while (itry1 < itry_max && bad) {
          ...
          while (itry < itry_max) {
            ...
            X = inuclRndm()*TM[icase];
            Ptest = (X/Xmax)*GXExp(-2.*u[icase]*Xmax + 2.*std::sqrt(u[icase]*(TM[icase] - X)));
            if (inuclRndm() < Ptest) {
              S = X + V[icase];
              break;
            }
