...
//
//
// $Id: GXNucleiPropertiesTheoreticalTableA.cc 91885 2015-08-10 07:05:56Z gcosmo $
//
// 
// ------------------------------------------------------------
...
G4int GXNucleiPropertiesTheoreticalTable::GetIndex(G4int Z, G4int A) 
{

  if(A>339) {
    G4Exception("GXNucleiPropertiesTheoreticalTable::GetIndex",
		"PART202",
...
G4double GXNucleiPropertiesTheoreticalTable::GetMassExcess(G4int Z, G4int A) 
{
...
G4double GXNucleiPropertiesTheoreticalTable::GetBindingEnergy(G4int Z, G4int A)
{
  G4int i=GetIndex(Z, A);
  if (i >= 0){
...
G4double  GXNucleiPropertiesTheoreticalTable::GetAtomicMass(G4int Z, G4int A)
{
  G4int i=GetIndex(Z, A);
  if (i >= 0) {
...
G4double  GXNucleiPropertiesTheoreticalTable::GetNuclearMass(G4int Z, G4int A)
{
  G4int i=GetIndex(Z, A);
  if (i >= 0) {
...
G4double GXNucleiPropertiesTheoreticalTable::ElectronicBindingEnergy(G4int Z) {
  const G4double ael = 1.433e-5*MeV; // electronic-binding constant
  return ael*std::pow(G4double(Z),2.39);
}

G4bool GXNucleiPropertiesTheoreticalTable::IsInTable(G4int Z, G4int A)
{
