...
// 20150619  M. Kelsey -- Replace std::exp with GXExp
...
void G4IntraNucleiCascader::generateCascade() {
  ...
  while (!cascad_particles.empty() && !model->empty()) {
	...
	if (KE < Q*coulombBarrier) {
	  ...
	  if (KE > 0.0001) CBP = GXExp(-0.0181*0.5*tnuclei->getZ()*
				       (1./KE - 1./coulombBarrier)*
...
G4bool G4IntraNucleiCascader::finishCascade() {
  ...
  if (theRecoilMaker->goodNucleus()) {
    ...
    GXFragment* recoilFrag = theRecoilMaker->makeRecoilFragment();
    if (!recoilFrag) {
...
decayTrappedParticle(const G4CascadParticle& trapped) {
  ...
  GXDecayTable* unstable = trappedP.getDefinition()->GetDecayTable();
  ...
  GXDecayProducts* daughters = unstable->SelectADecayChannel()->DecayIt( trappedP.getDefinition()->GetPDGMass() );
  ...
  for (G4int i=0; i<daughters->entries(); i++) {
    GXDynamicParticle* idaug = (*daughters)[i];
