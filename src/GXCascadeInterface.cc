...
// ********************************************************************
//
// $Id: GXCascadeInterface.cc 71719 2013-06-21 00:01:54Z mkelsey $
//
...
//Exclude by commented out of Propagate
...
GXCascadeInterface::~GXCascadeInterface() {
  clear();
...
void GXCascadeInterface::ModelDescription(std::ostream& outFile) const
{
...
void GXCascadeInterface::DumpConfiguration(std::ostream& outFile) const {
  G4CascadeParameters::DumpConfiguration(outFile);
}

void GXCascadeInterface::clear() {
  bullet=0;
  target=0;
}
...
void GXCascadeInterface::Initialize() {
  GXParticleDefinition* pn = G4UnboundPN::Definition();
  GXParticleDefinition* nn = G4Dineutron::Definition();
  GXParticleDefinition* pp = G4Diproton::Definition();

GXProton::Definition();
GXNeutron::Definition();
GXPionPlus::Definition();
GXPionMinus::Definition();
GXPionZero::Definition(); 
GXGamma::Definition();
GXKaonPlus::Definition(); 
GXKaonMinus::Definition();
GXKaonZero::Definition(); 
GXKaonZeroLong::Definition(); 
GXKaonZeroShort::Definition(); 
GXAntiKaonZero::Definition(); 
GXLambda::Definition(); 
GXSigmaPlus::Definition(); 
GXSigmaZero::Definition(); 
GXSigmaMinus::Definition();
GXXiZero::Definition(); 
GXXiMinus::Definition();
GXOmegaMinus::Definition(); 
GXDeuteron::Definition(); 
GXTriton::Definition();
GXHe3::Definition(); 
GXAlpha::Definition(); 
...
void GXCascadeInterface::useCascadeDeexcitation() {
  collider->useCascadeDeexcitation();
}

void GXCascadeInterface::usePreCompoundDeexcitation() {
...
void GXCascadeInterface::SetVerboseLevel(G4int verbose) {
...
G4bool GXCascadeInterface::IsApplicable(const GXHadProjectile& aTrack,
					GXNucleus& /* theNucleus */) {
  return IsApplicable(aTrack.GetDefinition());  
}

G4bool GXCascadeInterface::IsApplicable(const GXParticleDefinition* aPD) const {
  if (aPD->GetAtomicMass() > 1*CLHEP::GeV) return true;		// Nuclei are okay
...
GXCascadeInterface::ApplyYourself(const GXHadProjectile& aTrack, 
  ...
  if (aTrack.GetKineticEnergy() < 0.) {
    G4cerr << " >>> GXCascadeInterface got negative-energy track: "
...
GXHadFinalState* 
GXCascadeInterface::NoInteraction(const GXHadProjectile& aTrack,
				  GXNucleus& /*theNucleus*/) {
  if (verboseLevel) 
    G4cout << " >>> GXCascadeInterface::NoInteraction" << G4endl;
...
G4bool GXCascadeInterface::createBullet(const GXHadProjectile& aTrack) {
  const GXParticleDefinition* trkDef = aTrack.GetDefinition();
  G4int bulletType = 0;			// For elementary particles
  G4int bulletA = 0, bulletZ = 0;	// For nucleus projectile

  if (trkDef->GetAtomicMass() <= 1) {
    //bulletType = G4InuclElementaryParticle::type(convertG4PD2GXPD(trkDef));
  ...
  if (0 == bulletType && 0 == bulletA*bulletZ) {
    if (verboseLevel) {
      G4cerr << " GXCascadeInterface: " << trkDef->GetParticleName()
...
//TK 171210 Ignore tiny amount of Px and Py (They should be ZERO but they don't)
projectileMomentum.setPx(0.0);
projectileMomentum.setPy(0.0);
//TK 171210 END
...
G4bool GXCascadeInterface::createTarget(GXNucleus& theNucleus) {
  return createTarget(theNucleus.GetA_asInt(), theNucleus.GetZ_asInt());
}

/*
G4bool GXCascadeInterface::createTarget(G4V3DNucleus* theNucleus) {
  return createTarget(theNucleus->GetMassNumber(), theNucleus->GetCharge());
}
*/

G4bool GXCascadeInterface::createTarget(G4int A, G4int Z) {
  if (A > 1) {
...
GXDynamicParticle* GXCascadeInterface::
makeDynamicParticle(const G4InuclElementaryParticle& iep) const {
  G4int outgoingType = iep.type();
  
  if (iep.quasi_deutron()) {
    G4cerr << " ERROR: GXCascadeInterface incompatible particle type "
	   << outgoingType << G4endl;
    return 0;
  }
  
  // Copy local GXDynPart to public output (handle kaon mixing specially)
  if (outgoingType == kaonZero || outgoingType == kaonZeroBar) {
    ...
    GXParticleDefinition* pd = GXKaonZeroShort::Definition();
    //if (inuclRndm() > 0.5) pd = GXKaonZeroLong::Definition();
    if ( CLHEP::HepUniformRand() > 0.5) pd = GXKaonZeroLong::Definition();
    
    return new GXDynamicParticle(pd, momDir, ekin);
  } else {
    return new GXDynamicParticle(iep.getDynamicParticle());
  }
...
GXDynamicParticle* 
GXCascadeInterface::makeDynamicParticle(const G4InuclNuclei& inuc) const {
  if (verboseLevel > 2) {
    G4cout << " Nuclei fragment: \n" << inuc << G4endl;
  }
  
  // Copy local GXDynPart to public output 
  return new GXDynamicParticle( inuc.getDynamicParticle() );
}


// Transfer Bertini internal final state to hadronics interface

void GXCascadeInterface::copyOutputToHadronicResult() {
  if (verboseLevel > 1)
    G4cout << " >>> GXCascadeInterface::copyOutputToHadronicResult" << G4endl;
...
//G4cout << makeDynamicParticle(*ipart)->GetDefinition()->GetParticleName() << " " << makeDynamicParticle(*ipart)->Get4Momentum() << " " <<  makeDynamicParticle(*ipart)->GetKineticEnergy() << G4endl;
...
//G4cout << makeDynamicParticle(*ifrag)->GetDefinition()->GetParticleName() << " " << makeDynamicParticle(*ifrag)->Get4Momentum() << " " <<  makeDynamicParticle(*ifrag)->Get4Momentum() << " " << makeDynamicParticle(*ifrag)->GetKineticEnergy() << G4endl;
...
/*
G4ReactionProductVector* GXCascadeInterface::copyOutputToReactionProducts() {
  if (verboseLevel > 1)
    G4cout << " >>> GXCascadeInterface::copyOutputToReactionProducts" << G4endl;
...
void GXCascadeInterface::checkFinalResult() {
  balance->collide(bullet, target, *output);
...
G4bool GXCascadeInterface::coulombBarrierViolation() const {
...
G4bool GXCascadeInterface::retryInelasticProton() const {
...
G4bool GXCascadeInterface::retryInelasticNucleus() const {
...
void GXCascadeInterface::throwNonConservationFailure() {
  // NOTE:  Once GXHadronicException is changed, use the following line!
  // G4ExceptionDescription errInfo;
  std::ostream& errInfo = G4cerr;

  errInfo << " >>> GXCascadeInterface has non-conserving"
	  << " cascade after " << numberOfTries << " attempts." << G4endl;

  G4String throwMsg = "GXCascadeInterface - ";
  ...
  throw GXHadronicException(__FILE__, __LINE__, throwMsg);   // Job ends here!
}


/*
G4ParticleDefinition* GXCascadeInterface::convertGXPD2G4PD( const GXParticleDefinition* aGXPD ) const {

   G4ParticleDefinition* aG4PD=NULL; 
    
   //Create G4ParticleDefinition from GXParticleDefinition
   if ( GXParticleTable::GetParticleTable()->GetIonTable()->IsIon( aGXPD ) ) {
      //Ion is created from Z and A
      aG4PD = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIon( aGXPD->GetAtomicNumber() , aGXPD->GetAtomicMass() , 0.0 );
   } else {
      //Others (Elemental particles) created from name
      aG4PD = G4ParticleTable::GetParticleTable()->FindParticle( aGXPD->GetParticleName() );
   }
...
GXParticleDefinition* GXCascadeInterface::convertG4PD2GXPD( const G4ParticleDefinition* aG4PD ) const {

   GXParticleDefinition* aGXPD=NULL; 
    
   //Create GXParticleDefinition from G4ParticleDefinition
   if ( G4ParticleTable::GetParticleTable()->GetIonTable()->IsIon( aG4PD ) ) {
      //Ion is created from Z and A
      aGXPD = GXParticleTable::GetParticleTable()->GetIonTable()->GetIon( aG4PD->GetAtomicNumber() , aG4PD->GetAtomicMass() , 0.0 );
   } else {
      //Others (Elemental particles) created from name
      aGXPD = GXParticleTable::GetParticleTable()->FindParticle( aG4PD->GetParticleName() );
   }

   return aGXPD; 
}

G4DynamicParticle GXCascadeInterface::convertGXDP2G4DP( const GXDynamicParticle& aGXDP ) const {

   G4DynamicParticle* aG4DP=NULL; 
    
   G4ParticleDefinition* aG4PD = convertGXPD2G4PD( aGXDP.GetParticleDefinition() );
   aG4DP = new G4DynamicParticle( aG4PD , aGXDP.GetMomentum() );

   //TK Who will delete aG4DP?
   return *aG4DP; 
}
*/
