...
#include "GXHadDecayGenerator.hh"
#include "GXVHadDecayAlgorithm.hh"
...
GXHadDecayGenerator::GXHadDecayGenerator(Algorithm alg, G4int verbose)
...
GXHadDecayGenerator::GXHadDecayGenerator(GXVHadDecayAlgorithm* alg,
					 G4int verbose)
...
void GXHadDecayGenerator::ReportInvalidAlgorithm(Algorithm alg) const {
  if (verboseLevel) 
    G4cerr << "GXHadDecayGenerator: bad algorithm code " << alg << G4endl;

  throw GXHadronicException(__FILE__, __LINE__, "Invalid algorithm code");
}

void GXHadDecayGenerator::ReportMissingAlgorithm() const {
  if (verboseLevel) 
    G4cerr << "GXHadDecayGenerator: no algorithm specified" << G4endl;
...
void GXHadDecayGenerator::SetVerboseLevel(G4int verbose) {
  verboseLevel = verbose;
  if (theAlgorithm) theAlgorithm->SetVerboseLevel(verbose);
}

const G4String& GXHadDecayGenerator::GetAlgorithmName() const {
