...
bool isOutputInvalid(G4InuclParticle &bullet, G4InuclParticle &target, G4CollisionOutput &output)
{
  ...
  static G4CascadeCheckBalance checker("G4checker");
  static bool first = true;
  if(first) {
    checker.setLimits(5*CLHEP::perCent, 10*CLHEP::MeV/CLHEP::GeV);	// Bertini internal units
    checker.setVerboseLevel(debugLevel);
    first = false;
  }

  //G4InuclParticle *pcBullet = const_cast<G4InuclParticle*>(&bullet);
  //G4InuclParticle *pcTarget = const_cast<G4InuclParticle*>(&target);
  checker.collide(&bullet, &target, output);
  if(!checker.okay()) {
    G4cerr<<" *** E/P balance check failed!\n";
  }
...
template <typename T>
bool isOutputInvalid(GXInuclParticle<T> const& bullet, GXInuclParticle<T> const& target, GXCollisionOutput<T> const& output)
{
  // Quantities necessary for conditional block below
...
void RunG4ElementaryParticleCollider(GXTrack_v const& soaBullets, GXTrack_v const& soaTargets, G4CollisionOutput &output)
{
  ...
  for(size_t irep = 0; irep < nReps; ++irep) {
	...
	numberOfTries++;
      } while ( numberOfTries <= 20 && isOutputInvalid(bullet, target, output) );
				     ...
				     GXCollisionOutput<Real_v> &output)
{
  ...
  for(size_t irep = 0; irep < nReps; ++irep) {
	...
	numberOfTries++;
      } while ( numberOfTries <= 20 && isOutputInvalid<Real_v>(*pbullets, *ptargets, output) );
