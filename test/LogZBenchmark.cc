//
// File:    LogZBenchmark.cpp
// Purpose: Benchmark for the GXPowVec::LogZ() function
//
...
void OriginalLoop(int* z, size_t imax) {
  ...
  for(size_t irep = 0; irep < nReps; ++irep) {
    ...
    for(size_t i = 0; i < imax; ++i) {
      xval = oldpow->logZ(z[i]);
      xsum += xval;
      //std::cerr<<" original debug: "<< i <<' '<< z[i] <<'\t'<< xval <<' '<< xsum <<"\n";
    }
  }
  double elapsed = timer.Elapsed();
  std::cerr<<"  Original LogZ(): "<< xsum <<' '<< elapsed/1000. <<" msec\n";
}
...
void SIMDLoop(const char* testname, const int *__restrict__ z, size_t nobjs)
{
  ...
  for(size_t irep = 0; irep < nReps; ++irep) {
    sum = 0.;
    for(size_t i = 0; i < imax; ++i) {
      val = newpow->LogZ(zz[i]);
      sum += val;
      //std::cerr<<"SIMDLoop<"<< testname <<">: "<< i <<' '<< zz[i] <<'\t'<< val <<'\t'<< sum <<"\n";
    }
  }
  double elapsed = timer.Elapsed();
  std::cerr<< testname <<" LogZ(): "<< vecCore::ReduceAdd(sum) <<' '<< elapsed/1000 <<" msec\n";
}
...
int main()
{
  ...
  std::cerr<<"\n\n LogZ() Benchmark: nReps="<< nReps <<" and nvals="<< nvals <<"\n\n";
  ...
  for(size_t irep = 0; irep < nReps; ++irep) {
    ...
    for(size_t i = 0; i < nvals; ++i) {
      sc2x = newpow->StdLog(z[i]);
      sc2sum += sc2x;
      //std::cerr<<" scalarStd debug: "<< i <<' '<< z[i] <<'\t'<< sc2x <<'\t'<< sc2sum <<"\n";
    }
  }
  double scalar2Elapsed = timer.Elapsed();
  std::cerr<<" ScalarStd LogZ(): "<< sc2sum <<' '<< scalar2Elapsed/1000. <<" msec\n";
  ...
  for(size_t irep = 0; irep < nReps; ++irep) {
    ...
    for(size_t i = 0; i < nvals/vsize; ++i) {
      vecx = vecpow->StdLog(vz[i]);
      vecsum += vecx;
      //std::cerr<<" vectorStd debug: "<< i <<' '<< vz[i] <<'\t'<< vecx <<'\t'<< vecsum <<"\n";
    }
  }
  double vectorElapsed = timer.Elapsed();
  std::cerr<<" VectorStd LogZ(): "<< vecCore::ReduceAdd(vecsum) <<' '<< vectorElapsed/1000. <<" msec\n";
