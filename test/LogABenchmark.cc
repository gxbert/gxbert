//
// File:    LogABenchmark.cpp
// Purpose: Benchmark for the GXPowVec::LogA() function
//
...
void OriginalLoop(double* x, size_t imax) {
  ...
  for(size_t irep = 0; irep < nReps; ++irep) {
    ...
    for(size_t i = 0; i < imax; ++i) {
      xval = oldpow->logA(x[i]);
      xsum += xval;
      //std::cerr<<" original debug: "<< i <<' '<< a[i] <<' '<< x[i] <<'\t'<< xval <<' '<< xsum <<"\n";
    }
  }
  double elapsed = timer.Elapsed();
  std::cerr<<"  Original LogA(): "<< xsum <<' '<< elapsed/1000. <<" msec\n";
}
...
void SIMDLoop(const char* testname, const double *__restrict__ x, size_t nobjs)
{
  ...
  for(size_t irep = 0; irep < nReps; ++irep) {
    sum = 0.;
    for(size_t i = 0; i < imax; ++i) {
      val = newpow->LogA(xx[i]);
      sum += val;
      //std::cerr<<"SIMDLoop<"<< testname <<">: "<< i <<' '<< xx[i] <<' '<< aa[i] <<'\t'<< val <<'\t'<< sum <<"\n";
    }
  }
  double elapsed = timer.Elapsed();
  std::cerr<< testname <<" LogA(): "<< vecCore::ReduceAdd(sum) <<' '<< elapsed/1000 <<" msec\n";
}
...
int main()
{
  ...
  std::cerr<<"\n\n LogA() Benchmark: nReps="<< nReps <<" and nvals="<< nvals <<"\n\n";
  ...
  std::cerr<<" ScalarStd LogA(): "<< sc2sum <<' '<< scalar2Elapsed/1000. <<" msec\n";
  ...
  std::cerr<<" VectorStd LogA(): "<< vecCore::ReduceAdd(vecsum) <<' '<< vectorElapsed/1000. <<" msec\n";
