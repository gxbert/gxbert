//
// File:    Log10ABenchmark.cpp
// Purpose: Benchmark for the GXPowVec::Log10A() function
//
...
void OriginalLoop(double* x, size_t imax) {
  ...
  for(size_t irep = 0; irep < nReps; ++irep) {
    ...
    for(size_t i = 0; i < imax; ++i) {
      xval = oldpow->log10A(x[i]);
      xsum += xval;
      //std::cerr<<" original debug: "<< i <<' '<< a[i] <<' '<< x[i] <<'\t'<< xval <<' '<< xsum <<"\n";
    }
  }
  double elapsed = timer.Elapsed();
  std::cerr<<"  Original Log10A(): "<< xsum <<' '<< elapsed/1000. <<" msec\n";
}
...
void SIMDLoop(const char* testname, const double *__restrict__ x, size_t nobjs)
{
  ...
  for(size_t irep = 0; irep < nReps; ++irep) {
    sum = 0.;
    for(size_t i = 0; i < imax; ++i) {
      val = newpow->Log10A(xx[i]);
      sum += val;
      //std::cerr<<"SIMDLoop<"<< testname <<">: "<< i <<' '<< xx[i] <<' '<< aa[i] <<'\t'<< val <<'\t'<< sum <<"\n";
    }
  }
  double elapsed = timer.Elapsed();
  std::cerr<< testname <<" Log10A(): "<< vecCore::ReduceAdd(sum) <<' '<< elapsed/1000 <<" msec\n";
}
...
int main()
{
  ...
  std::cerr<<"\n\n Log10A() Benchmark: nReps="<< nReps <<" and nvals="<< nvals <<"\n\n";
  ...
  for(size_t irep = 0; irep < nReps; ++irep) {
    ...
    for(size_t i = 0; i < nvals; ++i) {
      sc2x = newpow->StdLog10(x[i]);
      sc2sum += sc2x;
      //std::cerr<<" scalarStd debug: "<< i <<' '<< a[i] <<' '<< x[i] <<'\t'<< sc2x <<'\t'<< sc2sum <<"\n";
    }
  }
  double scalar2Elapsed = timer.Elapsed();
  std::cerr<<" ScalarStd Log10A(): "<< sc2sum <<' '<< scalar2Elapsed/1000. <<" msec\n";
  ...
  for(size_t irep = 0; irep < nReps; ++irep) {
    ...
    for(size_t i = 0; i < nvals/vsize; ++i) {
      vecx = vecpow->StdLog10(vx[i]);
      vecsum += vecx;
      //std::cerr<<" vectorStd debug: "<< i <<' '<< va[i] <<' '<< vx[i] <<'\t'<< vecx <<'\t'<< vecsum <<"\n";
    }
  }
  double vectorElapsed = timer.Elapsed();
  std::cerr<<" VectorStd Log10A(): "<< vecCore::ReduceAdd(vecsum) <<' '<< vectorElapsed/1000. <<" msec\n";
